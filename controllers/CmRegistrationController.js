...
const { broadcast } = require('../WebSocketUtils');
...
const CmRegister = async (req, res) => {
    try {
        ...
        const { firstname, lastname, phonecode, mobile, email, gender, designation, organization, branch, username, passwords, crmId, crmName, createrid, createrrole } = req.body;
        ...
        if (crmId, crmName) {
            ...
            const [existingOrganization] = await mySqlpool.query("SELECT * FROM listoforganizations WHERE organizationname = ? ", [organization]);
            ...
            const [data] = await mySqlpool.query(
                ...
                [finalCMid, firstname, lastname, organizationid, organizationname, branch, crmId, crmName, phonecode, mobile, email, username, passwords, createrid, createrrole, date, time, imagePath, gender, extraind3, extraind10]
            );
            if (!data) {
            ...
            VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, '', '', '', '', '', ?, ?, ?, ?, '', '', '', '', '', '', '', '', '', '')`,
            [crmId, crmName, finalCMid, CmFullName, organizationid, organizationname, branch, createrid, createrrole, date, time ]
            );


      
            await mySqlpool.query(`UPDATE indicators SET cm = ? WHERE id = ?`, [nextcmid, id]);
