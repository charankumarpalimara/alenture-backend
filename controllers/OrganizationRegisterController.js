...
const organizationRegister = async (req, res) => {
    try {
        ...
        const { organizationname, branch, branchtype, phonecode, mobile, email, username, passwords, country, state, district, address, postalcode, createrid, createrrole } = req.body;
        if (!organizationname || !branch || !branchtype || !email || !username || !passwords || !country || !state || !district || !postalcode || !createrid || !createrrole) {
