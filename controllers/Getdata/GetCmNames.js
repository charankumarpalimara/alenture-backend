...
const GetCmNames = async (req, res) => {
    try {
        ...
        const [data] = await mySqlpool.query("SELECT firstname, lastname FROM listofcm WHERE organizationname = ? AND branch = ? ", [orgName,branch]);
        if (!data || data.length === 0) {
            return res.status(404).json({ error: "No CM names found" });
        }

        // Map each record to a full name
        const fullNames = data.map(cm => `${cm.firstname} ${cm.lastname}`);
