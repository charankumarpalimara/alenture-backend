...
async function broadcastExperienceCounts(mySqlpool, cmid, crmid) {
    ...
    const [[{ totalResolved }]] = await mySqlpool.query("SELECT COUNT(experienceid) AS totalResolved FROM experiences WHERE status = 'Resolved'");
    // For admin dashboard (new)
    const [[{ totalNew }]] = await mySqlpool.query("SELECT COUNT(experienceid) AS totalNew FROM experiences WHERE status = 'Open'");
    // For admin dashboard (pending)
    const [[{ totalPending }]] = await mySqlpool.query("SELECT COUNT(experienceid) AS totalPending FROM experiences WHERE status = 'Processing'");

    // For CRM dashboard (by crmid)
    let crmCounts = {};
    if (crmid) {
        const [[{ crmTotal }]] = await mySqlpool.query("SELECT COUNT(experienceid) AS crmTotal FROM experiences WHERE extraind1 = ?", [crmid]);
        const [[{ crmResolved }]] = await mySqlpool.query("SELECT COUNT(experienceid) AS crmResolved FROM experiences WHERE extraind1 = ? AND status = 'Resolved'", [crmid]);
        const [[{ crmNew }]] = await mySqlpool.query("SELECT COUNT(experienceid) AS crmNew FROM experiences WHERE extraind1 = ? AND status = 'Open'", [crmid]);
        const [[{ crmPending }]] = await mySqlpool.query("SELECT COUNT(experienceid) AS crmPending FROM experiences WHERE extraind1 = ? AND status = 'Processing'", [crmid]);
    ...
    if (wss && wss.clients) {
        wss.clients.forEach(client => {
            if (client.readyState === 1) {
                // Admin counts
                client.send(JSON.stringify({ type: 'adminCounts', total, totalResolved, totalNew, totalPending }));
