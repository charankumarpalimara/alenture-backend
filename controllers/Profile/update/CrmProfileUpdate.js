...
const updateCrmProfileByAdminAndHob = async (req, res) => {
    try {
        const { crmid, firstname, lastname, email, PhoneNo, gender, passwords, status } = req.body;
       ...
       if (!crmid || !firstname || !lastname || !email || !PhoneNo || !gender ||!status) {
        ...
        let updateFields = [firstname,  lastname, email, PhoneNo, gender, status];
        let sql = `UPDATE listofcrm SET firstname = ?, lastname = ?,  email = ?, mobile = ?, extraind2 = ?,  extraind7 = ?`;
        // If file is present, update extraind1 (profile image)
        if (req.file && req.file.filename) {
            sql += ', extraind1 = ?';
            updateFields.push(req.file.filename);
